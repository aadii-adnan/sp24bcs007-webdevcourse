...
// This function queries the database and returns all the stats
// we need for the sales dashboard.
// We put it in a function so both the page route and the API
// route can call the same code without repeating it.
// ============================================================
async function getSalesData() {
    ...
    // 2. Total revenue — add up the totalAmount of every order
    //    aggregate() lets us do math across all documents
    const revenueResult = await Order.aggregate([
        {
            $group: {
                _id: null,                          // group ALL orders together
                total: { $sum: '$totalAmount' }     // add up totalAmount field
            }
        }
    ]);
    // revenueResult is an array — take the first item's total, or 0 if no orders
    const totalRevenue = revenueResult.length > 0 ? Math.round(revenueResult[0].total) : 0;

    // 3. Average order value
    const avgOrderValue = totalOrders > 0 ? Math.round(totalRevenue / totalOrders) : 0;

    // 4. Count of delivered orders only
    const deliveredOrders = await Order.countDocuments({ status: 'delivered' });

    // 5. Top 5 selling products
    //    We unwind the items array (split each item into its own document),
    ...
    const topProducts = await Order.aggregate([
        { $unwind: '$items' },          // flatten the items array
        {
            $group: {
                ...
                totalSold: { $sum: '$items.quantity' }  // add up quantities
            }
        },
    ...
    // 6. 5 most recent orders
    const recentOrders = await Order.find()
        .sort({ createdAt: -1 })    // newest first
        .limit(5)
        .lean();

    return { totalRevenue, totalOrders, avgOrderValue, deliveredOrders, topProducts, recentOrders };
}
...
app.get("/contact-us", (req, res) => {
    res.render("contact-us", { activePage: 'contact' });
});
...
// ============================================================
// SALES DASHBOARD PAGE — GET /sales
// Renders the sales.ejs template with initial data from server
// ============================================================
app.get('/sales', isAdmin, async (req, res) => {
    try {
        ...
        res.render('sales', {
            ...
            activePage: 'sales',
            ...data    // spreads all the data fields into the template
        });
    } catch (err) {
        console.error(err);
...
// ============================================================
// SALES DATA API — GET /api/sales-data
// Called by jQuery every 10 seconds to get fresh numbers
